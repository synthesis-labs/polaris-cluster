...
public class TodoWsService {
    ...
    public static void startWsUpdatesService(StreamsBuilder builder, Map<String, String> serdeConfig) {
        ...
        // Consume the updates topic
        //
        final KStream<String, TodoUpdate> updates =
                builder.stream(todo_updates_topic, Consumed.with(stringSerde, todoUpdateSerde));

        // Emit updates to all websockets
        //
        updates.foreach((String websocketSession, TodoUpdate update) -> {
            WsUpdate wsUpdate = new WsUpdate();
            ...
            System.out.println("Pushing update to websocket");

            WsEndpoint.broadcast(wsUpdate);
        });
    }
